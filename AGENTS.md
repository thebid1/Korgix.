...
### PWA / Notifications
- **vite-plugin-pwa** generates the web app manifest and a single combined service worker (`src/sw.ts` → `dist/app/sw.js`, served at `/app/sw.js`)
- The **combined service worker** handles both Workbox precaching/runtime caching and FCM push/notification-click events
- **App lives under `/app/`**: Vite `base` is `/app/`, the SW scope and manifest `start_url`/`scope` are all `/app/`. The site root is the marketing landing page (from `appsite/`). Keep every absolute app path (`/app/sw.js`, `/app/icons/...`) consistent with this.
...
- **Install prompt** (`InstallPrompt.tsx`) captures the Android `beforeinstallprompt` event and triggers the native browser install prompt
- **PWA shortcuts & share target**: `/app/?action=add` opens the add-task view; the OS share target can pre-fill title/text/url
...
├── appsite/                  # Standalone marketing landing page (self-contained, deployed at site root)
│   ├── index.html            # App site markup + Formspree subscribe form
│   ├── styles.css            # App site styles
│   ├── animations.js         # GSAP scroll animations
│   ├── icons/                # App icon assets used by the landing page
│   ├── sw.js                 # Root-scope migration shim: unregisters the pre-/app PWA service worker
│   └── README.md             # App site preview instructions
...
├── public/
│   ├── manifest.json         # Static PWA manifest (also generated by vite-plugin-pwa in dist/)
...
- **URL layout:** The PWA builds to `dist/app/` (Vite `base: '/app/'`) and is served at `/app/`. The landing page files from `appsite/` are copied to the `dist/` root and served at `/`. `vercel.json` redirects the legacy `/appsite/*` path to `/`.
- **Root SW shim:** `appsite/sw.js` is copied to `dist/sw.js` (the site root). It exists solely to unregister the old root-scoped service worker from installs made before the app moved to `/app/` — it clears caches, unregisters itself, and reloads open clients. The real app worker lives at `/app/sw.js` and never controls the landing page.
...
- **Background/push payload:** Edit `functions/src/index.ts` for the server payload, and `src/sw.ts` for how the service worker renders the notification.
...
- PWA manifest and Workbox caching are generated by `vite-plugin-pwa` at build time from `src/sw.ts` to `dist/app/sw.js`; `main.tsx` registers `/app/sw.js` manually
