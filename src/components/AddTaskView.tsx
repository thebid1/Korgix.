...
export const AddTaskView = ({ onClose, onOpenRecurrence }: AddTaskViewProps) => {
  ...
  const quickAdd = (minutes: number) => {
    ...
    if (!startTime) setStartTime(formatForInput(baseStart));
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ background: 'var(--bg)' }}>
      <div className="flex items-center justify-between px-5 pt-12 pb-4">
        <button onClick={onClose} className="w-10 h-10 rounded-full flex items-center justify-center active:scale-90" style={{ background: 'var(--surface)' }}>
      ...
      <div className="flex-1 px-5 pb-8 overflow-y-auto">
        <div className="mb-6">
          ...
          <input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="e.g. Draft launch announcement" autoFocus
        ...
        <div className="grid grid-cols-2 gap-4 mb-6">
          <div>
            <label className="text-[11px] font-bold tracking-widest uppercase mb-2 block" style={{ color: 'var(--text-muted)' }}>Starts</label>
            <div className="flex items-center gap-2 px-3 py-3 rounded-xl" style={{ background: 'var(--surface)' }}>
              <Clock size={16} style={{ color: 'var(--text-secondary)' }} />
              <input type="datetime-local" value={startTime} onChange={(e) => setStartTime(e.target.value)}
                className="bg-transparent text-sm font-medium outline-none flex-1 min-w-0" style={{ color: 'var(--text)' }} />
            </div>
          </div>
          <div>
            <label className="text-[11px] font-bold tracking-widest uppercase mb-2 block" style={{ color: 'var(--text-muted)' }}>Ends</label>
            <div className="flex items-center gap-2 px-3 py-3 rounded-xl" style={{ background: 'var(--surface)' }}>
              <Clock size={16} style={{ color: 'var(--text-secondary)' }} />
        ...
        <div className="rounded-2xl p-4 mb-6" style={{ background: 'var(--surface)' }}>
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-sm font-bold" style={{ color: 'var(--text)' }}>{duration > 0 ? durationLabel : '—'}</span>
          </div>
          <div className="flex gap-2">
            {[{ label: '15m', min: 15 }, { label: '30m', min: 30 }, { label: '1h', min: 60 }, { label: '2h', min: 120 }].map((opt) => (
              <button key={opt.label} type="button" onClick={() => quickAdd(opt.min)}
                className="flex-1 py-2.5 rounded-xl text-sm font-semibold active:scale-95" style={{ background: 'var(--bg)', color: 'var(--text)' }}>
                {opt.label}
              </button>
            ))}
          </div>
        ...
        <div className="mb-6">
          <label className="text-[11px] font-bold tracking-widest uppercase mb-2 block" style={{ color: 'var(--text-muted)' }}>Notes</label>
        ...
        <div className="flex items-center gap-2.5 px-4 py-3 rounded-xl" style={{ background: 'var(--surface)' }}>
